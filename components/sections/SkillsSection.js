    ...
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#0a0f1f] via-[#111827] to-[#050816] text-white px-6 py-20 flex items-center">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {skills.map((skill, i) => (
            <Reveal key={skill.name} delay={i * 0.05} y={20}>
              <SkillCard
                name={skill.name}
                icon={skill.icon}
                color={skill.color}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16">
          {[
            ...
            <Reveal key={item.label} delay={i * 0.08}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center backdrop-blur-md hover:border-cyan-400/40 hover:shadow-[0_0_20px_rgba(34,211,238,0.15)] transition duration-300">
